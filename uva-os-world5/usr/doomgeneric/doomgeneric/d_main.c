...
void D_DoomMain (void)
{
    int p;
    char file[256];
    // fxl: a bug? when demolumpname is assigned to defdemoname,  
    // and referenced in G_CheckDemoStatus() (via I_AtExit()), defdemoname points
    // to garbage. does the stack frame here still exist? 
    // change to "static" seems to fix the problem. char
    // demolumpname[9];        // orig 
...
#endif
    {
        ...
        // M_SetConfigDir(NULL);
        // fxl: to diff against multiple doom instances...
        // TBD free these names at exit ... 
extern int getpid(void);        
        int pid = getpid(); 
        char *config_dir = malloc(64);
        snprintf(config_dir, 64, "./doomconfig-%d/", pid);
        M_SetConfigDir(config_dir);
    }
