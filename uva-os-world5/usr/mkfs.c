...
// fxl: very simple. only make files, no dirs
...
// fxl: in bitmap, mark [0,used) blocks to be in use
...
void        // fxl: append data (xp, bytes: n) to inode
iappend(uint inum, void *xp, int n)
{
  ...
  while(n > 0){   // n: # remaining bytes to write
    ...
    assert(fbn < MAXFILE);  // fxl: ~270KB filesize limit..
    if(fbn < NDIRECT){
...
die(const char *s)
{
  ...
  perror(s);
  printf("\033[1;31mPossible reason: a file under usr/build too large; or total files there too large\033[0m\n");
  printf("\033[1;31mPossible reason: a file under usr/build too large; or total files there too large\033[0m\n");
  printf("\033[1;31mPossible reason: a file under usr/build too large; or total files there too large\033[0m\n");
  exit(1);
}
