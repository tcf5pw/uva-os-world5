...
// disable most other functions. if needed, turn back on
...
#define SDL_InvalidParamError(x) \
    do {    \
        printf(x); \
        assert(0); \
    } while (0)
...
// fxl: NB: in table below, two differnet scancodes may have same SDL_keycode. (weird)
...
SDL_Keycode SDL_GetKeyFromScancode(SDL_Scancode scancode)
{
    if (((int)scancode) < SDL_SCANCODE_UNKNOWN || scancode >= SDL_NUM_SCANCODES) {
        // SDL_InvalidParamError("scancode"); // fxl: avoid failure, for lab5 quest1 subquest 1
        printf("%s: warning: bad scan code", __func__); 
        return 0;
    }
    
    // fxl: special case...
    if (scancode == SDL_SCANCODE_DELETE) return SDLK_DELETE; 
    
    // fxl: these scancodes are contig, cf sdl-scancode.h
    // also cf sdl-keycode.h
    if (scancode >= SDL_SCANCODE_CAPSLOCK && scancode <= SDL_SCANCODE_AUDIOFASTFORWARD)
...
const char *SDL_GetKeyName(SDL_Keycode key)
{
    ...
    if (key & SDLK_SCANCODE_MASK) { // fxl: note this...
