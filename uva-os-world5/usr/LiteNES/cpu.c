...
// CPU Lifecycle
// fxl: ppulate all cpu ops, names, address modes, and handlers
void cpu_init()
{
...
extern void printf(const char*, ...); // fxl

void cpu_run(long cycles)           // fxl: the cpu emu loop. run this many cycles then return (to exec ppu)
{
    static int once = 0;  // fxl: change this to 1 for op tracing
    while (cycles > 0) {
