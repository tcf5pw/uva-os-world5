...
//  bank0: gpios 0..31
#define ARM_GPIO_GPSET0		(ARM_GPIO_BASE + 0x1C)  // GPIO Pin Output Set
#define ARM_GPIO_GPCLR0		(ARM_GPIO_BASE + 0x28)  // GPIO Pin Output Clear
#define ARM_GPIO_GPLEV0		(ARM_GPIO_BASE + 0x34)  // GPIO Pin Level
#define ARM_GPIO_GPEDS0		(ARM_GPIO_BASE + 0x40)  // GPIO Pin Event Detect Status
...
#define ARM_GPIO_GPAREN0	(ARM_GPIO_BASE + 0x7C)  // GPIO Pin Async Rising Edge Detect
#define ARM_GPIO_GPAFEN0	(ARM_GPIO_BASE + 0x88)  // GPIO Pin Async Falling Edge Detect
// bank1: gpios 32..53
#define ARM_GPIO_GPSET1		(ARM_GPIO_BASE + 0x20)  // GPIO Pin Output Set
#define ARM_GPIO_GPCLR1		(ARM_GPIO_BASE + 0x2C)  // GPIO Pin Output Clear
#define ARM_GPIO_GPLEV1		(ARM_GPIO_BASE + 0x38)  // GPIO Pin Level
#define ARM_GPIO_GPEDS1		(ARM_GPIO_BASE + 0x44)  // GPIO Pin Event Detect Status
#define ARM_GPIO_GPREN1		(ARM_GPIO_BASE + 0x50)  // GPIO Pin Rising Edge Detect Enable
#define ARM_GPIO_GPFEN1		(ARM_GPIO_BASE + 0x5C)  // GPIO Pin Falling Edge Detect Enable
#define ARM_GPIO_GPHEN1		(ARM_GPIO_BASE + 0x68)  // GPIO Pin High Detect Enable
#define ARM_GPIO_GPLEN1		(ARM_GPIO_BASE + 0x74)  // GPIO Pin Low Detect Enable
#define ARM_GPIO_GPAREN1	(ARM_GPIO_BASE + 0x80)  // GPIO Pin Async Rising Edge Detect
#define ARM_GPIO_GPAFEN1	(ARM_GPIO_BASE + 0x8C)  // GPIO Pin Async Falling Edge Detect
...
// cf Circle bcm2835int.h   https://github.com/rsta2/circle/blob/master/include/circle/bcm2835int.h
...
#define IRQ_PENDING_2		(PBASE+0x0000B208)  // irq 32..63 from gpu side
    #define IRQ_PENDING_2_GPIO(x) (1<<(17+x))    // four GPIO banks, irq 49..52
#define FIQ_CONTROL		    (PBASE+0x0000B20C)
#define ENABLE_IRQS_1		(PBASE+0x0000B210)  // irq 0..31
    #define ENABLE_IRQS_1_USB (1<<9)
    #define ENABLE_IRQS_1_DMA(x) (1<<(16+x))      // dma channel 0..12
    #define ENABLE_IRQS_1_AUX (1<<29)
#define ENABLE_IRQS_2		(PBASE+0x0000B214)  // irq 32..63
    #define ENABLE_IRQS_2_GPIO(x) (1<<(17+x))   // four GPIO banks, irq 49..52
